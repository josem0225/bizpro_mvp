...
export default function TeaserPage() {
    ...
    return (
        <main className="min-h-screen relative font-sans text-white bg-[var(--navy-deep)] overflow-hidden">
            <ParticlesBackground />
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-4 py-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                    ...
                    <div className="bg-[var(--glass-bg)] border border-[var(--glass-border)] p-8 rounded-[var(--radius-lg)] text-center relative hover:bg-[rgba(255,255,255,0.02)] transition-colors h-full flex flex-col">
                        <h3 className="text-xl font-bold">{pricing.stepByStep.title[language]}</h3>
                        <div className="my-5 text-[2.5rem] font-bold leading-none">
                        ...
                        <Link href="/dashboard" className="w-full py-3 rounded-xl font-semibold bg-[var(--glass-bg)] border border-[var(--glass-border)] text-[var(--white)] hover:bg-[rgba(255,255,255,0.1)] hover:border-white transition-all block">
                            {pricing.stepByStep.cta[language]}
                        </Link>
                    </div>

                    {/* OPTION B: Startup Pack (Featured) */}
                    <div className="bg-[rgba(23,19,77,0.8)] border border-[var(--blue-electric)] p-8 rounded-[var(--radius-lg)] text-center relative z-[2] shadow-[0_0_40px_rgba(59,130,246,0.2)] transform scale-105 h-full flex flex-col">
                        ...
                        <div className="inline-block bg-[rgba(16,185,129,0.1)] text-[var(--success)] text-sm font-semibold px-2 py-1 rounded mb-4 self-center">
                            {pricing.startup.save[language]} {fmt(packages.startup.savings)}
                        </div>
                        <h3 className="text-2xl font-bold">{pricing.startup.title[language]}</h3>
                        ...
                        <ul className="text-left mb-8 space-y-3 text-[var(--text-gray)] text-sm flex-grow">
                            ...
                            <li className="flex items-center gap-2.5"><Check className="text-[var(--success)] w-4 h-4" /> {language === 'es' ? "Acceso comunidad" : "Community access"}</li>
                        </ul>
                        <button className="w-full py-4 rounded-xl font-bold bg-[var(--blue-electric)] text-white shadow-[0_4px_15px_rgba(59,130,246,0.4)] hover:bg-[#2563EB] transition-all flex items-center justify-center gap-2" onClick={() => window.location.href = '/dashboard'}>
